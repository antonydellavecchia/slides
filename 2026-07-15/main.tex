...
\begin{frame}{Serializing Algebraic Data}
  ...
  \item Elements sharing a parent ring should have the same parent ring on load.
  \end{itemize}
\end{frame}

%% Slide 2 --- mrdi file format
\begin{frame}[fragile]{The \mrdi File Format [24 DV - Joswig - Lorenz]}
  \vspace*{-0.5cm}
  \begin{tabular}{cl}
    \begin{tabular}{c}
...
%
  ...
  \item Selection of different serializers allow users to alter subtrees.
  \item Parametrized \texttt{SerializerState\{S\}}.
  \item Multiple dispatch can select the encoding per context.
  \end{itemize}
  \bigskip
...
\begin{frame}{\texttt{Distributed.jl} Integration}
  \begin{columns}[T]
    ...
    \begin{column}{0.58\textwidth}
      ...
      \begin{itemize}
      \item Handles sending context to workers.
      \item \Julia's \texttt{remotecall} and \texttt{pmap} works directly with \OSCAR types.
...
\begin{frame}{Motivating Examples}
  \textbf{Finding generators for Vanishing ideals}
  Example: Degree five components of the vanishing ideal for $\sigma_4(\PP^3 \times \PP^3 \times \PP^3)$
  \begin{itemize}
    \item Multigraded Implicitization [26 Cummings-Hollering]
...
\begin{frame}{Future Work}
  ...
  \item Writing to binary formats to reduce communication overhead.
  \item Make framework available as a standalone \Julia package.
  \end{itemize}
  \vfill
  \pause
  \begin{center}
